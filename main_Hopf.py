...
def generate_data_Hopf(N_f, constant_dt):
    ...
    def visualize(real_X_star, learned_X_star, learned_S):
        ...
        for k in range(0, learned_S):
            colorline3d(ax, real_X_star[k, :, 0], real_X_star[k, :, 1], real_X_star[k, :, 2], cmap=plt.cm.seismic)
        ax.grid(False)
        ax.set_xlabel('$\mu$')
        ax.set_ylabel('$x$')
        ax.set_zlabel('$y$')
        ...
        if not args.viz:
            plt.savefig('./figures/' + file_base_name_without_extension + time.strftime("%Y%m%d-%H%M%S") + 'real' +'.png', bbox_inches='tight')
        else:
            plt.show()
        plt.clf()
        fig = plt.figure()
        ...
        ax.w_zaxis.set_pane_color((1.0, 1.0, 1.0, 1.0))
        for k in range(0, learned_S):
            colorline3d(ax, learned_X_star[k, :, 0], learned_X_star[k, :, 1], learned_X_star[k, :, 2],
                        cmap=plt.cm.seismic)
        ax.grid(False)
        ax.grid(False)
        ax.set_xlabel('$\mu$')
        ...
        if not args.viz:
            plt.savefig('./figures/' + file_base_name_without_extension + time.strftime("%Y%m%d-%H%M%S") + 'prediction' + '.png', bbox_inches='tight')
        else:
            plt.show()
        # dim = u_real.shape[2]
        # print('dimension', dim)
...
def one_round(N_f, constant_dt, interval_number, method, model_name, train_method):
    ...
    layers = [net_dim, 128, net_dim]  #network info (one hidden layer with 128 nodes)
    ...
    if train_method == "batch":
        ...
        model.train(epochs=1, nIter=20000)  # train the model with full data
    else:
        return None
    ...
    if model_name == "Hopf":
        ...
        visualize_function(real_X_star, learned_X_star, learned_S)
    else:
