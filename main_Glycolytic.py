...
def generate_data_Multiscale(N_f, constant_dt):
    ...
    def visualize(sol_t, u_real, u_pred):
        ...
        for i in range(dim):
            ...
            if not args.viz:
                plt.savefig('./figures/' + file_base_name_without_extension + timestr + '_' + 'multiscale' + str(i) + '.png')
            else:
                plt.show()
            plt.clf()
...
def generate_data_Glycolytic(N_f, constant_dt):
    ...
    def visualize(sol_t, u_real, u_pred):
        ...
        for i in range(dim):
            plt.plot(sol_t, u_real[:, i:i + 1], label='real', linewidth = 4)
            plt.plot(sol_t, u_pred[:, i:i + 1],'--', label='predicted', linewidth = 4)
            ...
            if not args.viz:
                plt.savefig('./figures/' + file_base_name_without_extension + time.strftime("%Y%m%d-%H%M%S") + '_' + str(i) + '.png')
            else:
                plt.show()
            plt.clf()
...
def one_round(N_f, constant_dt, interval_number, method, model_name, train_method):
    ...
    layers = [net_dim, 128, net_dim]  #network info (two hidden layer with 128 nodes)
    ...
    if train_method == "batch":
        ...
        model.train(epochs=1, nIter=10000)  # train the model with full data
    else:
        return None
